...
sub vcl_recv {
  ...
  if (req.method == "PURGE" || req.method == "SOFTPURGE" || req.method == "BAN") {
    ...
    # Surrogate Key Purging using x-key
    if (req.http.surrogate-key) {
      set req.http.xkey = req.http.surrogate-key;
    }
    if (req.http.xkey) {
      set req.http.x-purge-keys = xkey.softpurge(req.http.xkey);

      if (std.integer(req.http.x-purge-keys, 0) != 0) {
        return (synth(200, "Purged " + req.http.x-purge-keys + " keys"));
      } else {
  ...
  if (req.method == "TRANCE") {
    return (synth(405, "DUNZ DUNZ DUNZ DUNZ DUNZ"));
  }
...
sub vcl_backend_response {
  ...
  # Save the original URL without ports.
  set beresp.http.X-URL = bereq.http.host + bereq.url; 

if (beresp.http.Surrogate-Control) {
    set beresp.ttl = std.duration(regsub(beresp.http.Surrogate-Control,"^.*max-age=(\d+).*$","\1s"),  beresp.ttl);
    set beresp.grace = std.duration(regsub(beresp.http.Surrogate-Control,"^.*max-age=\d+\+(\d+).*$","\1s"),beresp.grace);
  }

  /* Set X Key to HashTwo or surrogate key for legacy reasons */
  if (!beresp.http.xkey) {
    if (beresp.http.Surrogate-Key) {
      set beresp.http.xkey = beresp.http.Surrogate-Key;
    } elsif (beresp.http.X-HashTwo) {
      set beresp.http.xkey = beresp.http.X-HashTwo;
    }
  }
...
<html>
  ...
  <body>
    <h1>URL is "} + bereq.url + {"</h1>
    <h1>There seems to be trouble with our backend unfortunatly</h1>
...
<html>
  <head>
    <title>FUXXXORS"} + resp.status + " " + resp.reason + {"</title>
  </head>
  <body>
    <h1>URL is "} + req.url + {"</h1>
    <h1>Error "} + resp.status + " " + resp.reason + {"</h1>
    <p>"} + resp.reason + {"</p>
    <h3>Guru Meditation:</h3>
    <p>XID: "} + req.xid + {"</p>
    <hr>
    <p>Varnish Snuskeserver</p>
  </body>
</html>
"} );
  return (deliver);
